...
-- how to check how many table are there in my database 
show tables;
-- the following code will show all the data in myemp table 
select * from myemp;
-- what if i dont want to see entire dataset rather only 10 or 20 records 
select * from myemp limit 10;
-- now i want to see only those column which i want to see 
select emp_id,first_name,last_name from myemp limit 10;
-- now i want to see columns with applied calculation(i want to salary of employe with 20percent of it)
select emp_id,first_name,last_name,SALARY,0.2*salary from myemp limit 10;



-- concatination
select emp_id,concat(first_name,' ',last_name) as name,salary from myemp limit 30;


-- distinct clause (this is show only unique values)
select distinct dep_id from myemp; 
select distinct dep_id from myemp order by dep_id; -- this will print as acending order 
select distinct dep_id from myemp order by dep_id desc; 
select distinct dep_id from myemp order by dep_id asc; 
 select distinct MGR_ID, DEP_ID from myemp order by mgr_id ; -- this will order according to manager id column 
 select distinct fname from person;
  ...
  -- where clause 
  -- based on condition if you want to fetch records 
  select * from myemp where salary>15000;
  
  
  -- fuctions and operators
  -- comparison
  -- <,>,<=>(null),betweent....and...,is null,is not null,or,like,not()..like,<=,>=,not between.....and...not 
  select * from myemp where salary between 10000 and 15000 order by salary; -- this will print salary between 10000 and 15000 and it will sort in acsending order
  select * from myemp where JOB_ID='st_man';
  select * from myemp where JOB_ID='st_man'and salary>=6000;
    select * from myemp where JOB_ID='st_man'and salary>=6000 or salary>10000;
select * from myemp where dep_id= 50 or dep_id= 60 or dep_id= 70;
 -- you can same code using in operator 
 select * from myemp where dep_id in (50,60,70);
 -- you can use except function using not in 
 select * from myemp where dep_id  not in (50,60,70);
 
 
/* we have two types of fuction called
1)row level fuction- each and every record will give output (example now,year fuction) 
2)group/aggregate fuctions-for entire table it will give one output (minimum maximum some average count and standard deviation fuction)
and concate is text level fuction */
